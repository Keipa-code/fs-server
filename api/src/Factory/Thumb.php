...
class Thumb
{
    ...
    public function resizeCanvas($width, $height, $bg = array())
    {
        ...
        if ($width >= $this->width && $height >= $this->height) {
            $seze = array((int)ceil(($width - $this->width) / 2), (int)ceil(($height - $this->height) / 2), $this->width, $this->height);
        } elseif ($width >= $this->width) {
            $seze = array((int)ceil(($width - $tw) / 2), 0, (int)ceil($height / ($this->height / $this->width)), $height);
        } elseif ($height >= $this->height) {
            $seze = array(0, (int)ceil(($height - $th) / 2), $width, (int)ceil($width / ($this->width / $this->height)));
        } elseif ($tw < $width) {
            $seze = array((int)ceil(($width - $tw) / 2), (int)ceil(($height - $height) / 2), $tw, $height);
        } else {
            $seze = array(0, (int)ceil(($height - $th) / 2), $width, $th);
        }
        ...
        if ($this->width != $width && $this->height != $height) {
            ...
            if ($top_defined || $bottom_defined || $right_defined || $left_defined) {
                ...
                if ($top_defined == false && $right_defined && $bottom_defined && $left_defined) {
                    // top
                    if ($width >= $this->width && $height >= $this->height) {
                        $seze = array((int)ceil(($width - $this->width) / 2), 0, $this->width, $this->height);
                    } elseif ($width >= $this->width) {
                        $seze = array((int)ceil(($width - ($height / ($this->height / $this->width))) / 2), 0, (int)ceil($height / ($this->height / $this->width)), $height);
                    } elseif ($height >= $this->height) {
                        $seze = array(0, 0, $width, (int)ceil($width / ($this->width / $this->height)));
                    } elseif ($tw < $width) {
                        $seze = array((int)ceil(($width - $tw) / 2), 0, $tw, $height);
                    } else {
                        $seze = array((int)ceil(($width - $width) / 2), 0, $width, $th);
                    }
                    ...
                    if ($width >= $this->width && $height >= $this->height) {
                        ...
                        $seze = array(0, 0, (int)ceil($height / ($this->height / $this->width)), $height);
                    } elseif ($height >= $this->height) {
                        $seze = array(0, 0, $width, (int)ceil($width / ($this->width / $this->height)));
                    } elseif ($tw < $width) {
                    ...
                    if ($width >= $this->width && $height >= $this->height) {
                        ...
                        $seze = array($width - $tw, 0, (int)ceil($height / ($this->height / $this->width)), $height);
                    } elseif ($height >= $this->height) {
                        $seze = array(0, 0, $width, (int)ceil($width / ($this->width / $this->height)));
                    } elseif ($tw < $width) {
                    ...
                    if ($width >= $this->width && $height >= $this->height) {
                        ...
                        $seze = array(0, $height - $th, $width, (int)ceil($width / ($this->width / $this->height)));
                    } elseif ($tw < $width) {
                        $seze = array(0, 0, $tw, $height);
                    } else {
                    ...
                    // right
                    if ($width >= $this->width && $height >= $this->height) {
                        $seze = array($width - $this->width, (int)ceil(($height - $this->height) / 2), $this->width, $this->height);
                    } elseif ($width >= $this->width) {
                        $seze = array($width - $tw, 0, (int)ceil($height / ($this->height / $this->width)), $height);
                    } elseif ($height >= $this->height) {
                        $seze = array(0, $height - $th, $width, (int)ceil($width / ($this->width / $this->height)));
                    } elseif ($tw < $width) {
                        $seze = array($width - $tw, (int)ceil(($height - $height) / 2), $tw, $height);
                    } else {
                        $seze = array(0, (int)ceil(($height - $th) / 2), $width, $th);
                    }
                    ...
                    // bottom
                    if ($width >= $this->width && $height >= $this->height) {
                        $seze = array((int)ceil(($width - $this->width) / 2), $height - $this->height, $this->width, $this->height);
                    } elseif ($width >= $this->width) {
                        $seze = array((int)ceil(($width - ($height / ($this->height / $this->width))) / 2), 0, (int)ceil($height / ($this->height / $this->width)), $height);
                    } elseif ($height >= $this->height) {
                        $seze = array(0, $height - $th, $width, (int)ceil($width / ($this->width / $this->height)));
                        ...
                        $seze = array((int)ceil(($width - $width) / 2), $height - $th, $width, $th);
                    }
                } elseif ($top_defined && $right_defined && $bottom_defined && $left_defined == false) {
                    // left
                    if ($width >= $this->width && $height >= $this->height) {
                        $seze = array(0, (int)ceil(($height - $this->height) / 2), $this->width, $this->height);
                    } elseif ($width >= $this->width) {
                        $seze = array(0, 0, (int)ceil($height / ($this->height / $this->width)), $height);
                    } elseif ($height >= $this->height) {
                        $seze = array(0, (int)ceil(($height - $th) / 2), $width, (int)ceil($width / ($this->width / $this->height)));
                    } elseif ($tw < $width) {
                        $seze = array(0, (int)ceil(($height - $height) / 2), $tw, $height);
                    } else {
                        $seze = array(0, (int)ceil(($height - $th) / 2), $width, $th);
                    }
